package br.com.paulo.animais.utils;

import br.com.paulo.animais.properties.PropertiesBundle;
import java.io.PrintWriter;
import java.io.StringWriter;
import java.util.MissingResourceException;
import javafx.scene.control.Alert;

/**
 *
 * @author paulo
 */
public class AlertasUtils {
    
    public static void exibirErro(String mensagem) {
        montarMensagem(new Alert(Alert.AlertType.ERROR), mensagem);
    }
    
    public static void exibirErro(Exception ex) {
        final StringWriter sw = new StringWriter();
        final PrintWriter pw = new PrintWriter(sw, true);
        ex.printStackTrace(pw);
        montarMensagem(new Alert(Alert.AlertType.ERROR), sw.getBuffer().toString());
    }
    
    public static void exibirInformacao(String mensagem) {
        montarMensagem(new Alert(Alert.AlertType.INFORMATION), mensagem);
    }
    
    private static void montarMensagem(Alert alert, String mensagem) {
        alert.setTitle(PropertiesBundle.getProperty("TITULO_ERRO"));
        alert.setHeaderText(null);
        
        try {
            alert.setContentText(PropertiesBundle.getProperty(mensagem));
        } catch (MissingResourceException ex) {
            int tamanhoMaximoMensagem = 900;
            if (mensagem.length() > tamanhoMaximoMensagem) {
                mensagem = mensagem.substring(0, tamanhoMaximoMensagem);
            }
            alert.setContentText(mensagem);
        }
        
        alert.showAndWait();
    }
    
}
